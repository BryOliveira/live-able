function formatSalaryRange(job: any): string {
  ...
  const min = toNumber(min_salary);
  const max = toNumber(max_salary);
...
export default function JobView({ job }: { job: any }) {
  if (!job) {
    return (
      <div id='job-view'>
  ...
  return (
    <div id='job-view'>
      ...
      <div className='job-subheader'><p><span className='greened'>Location:</span> {job.loc_city}, {job.loc_state}</p><p><span className='greened'>State&apos;s Median House Price:</span> ${formatPrice(job.home_prices?.median_house_price)}</p></div>
      <div className='desc'>
